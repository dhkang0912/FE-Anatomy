...
- [Chapter 5. 클로저](#chapter-5-클로저)
  ...
  - [5-4. 정리](#5-4-정리)
    - [5-4-1. 스코프 체인과 클로저 구분하기!](#5-4-1-스코프-체인과-클로저-구분하기)
- [Q \& A](#q--a)
...
## 5-2. 클로저와 메모리 관리
클로저는 **의도적으로** 외부 함수의 **지역 변수**를 **함수 내부에서 계속 참조**하도록 만듦  
- 클로저는 메모리 누수를 일으킨다?! ❌
  - 메모리를 더 오래 유지하도록 함
...
```
- fruit는 외부 변수지만, click 이벤트의 콜백 함수 내부에서 사용됨
- addEventListener는 콜백 함수에 **이벤트 객체(event)** 를 첫 번째 인자로 자동 전달
  - function()은 익명 함수이고, event 파라미터를 명시적으로 선언X → alert에 영향 없음
  - forEach가 호출될 때마다 별개의 실행 컨텍스트가 만들어지고, 각각의 fruit 값이 클로저에 저장됨
  - 따라서 외부 변수 fruit를 기억하고 그대로 출력할 수 있음
...
- ✨ 고차함수를 사용하는 이유
  -  콜백에 넘길 시점에 인자를 미리 바인딩한 **새로운 함수**를 만들어두기 위해
  -  즉, 동작을 미루고 그 시점에 필요한 값을 기억한 함수(클로저)를 만들기 위해
  -  유지보수성과 명확성 확보
```js
...
```
- alertFruitBuilder(fruit)를 실행하면 새 함수 반환 → addEventListener에 콜백으로 넘김
  - 반환된 함수는 fruit 값을 기억하고 있음(클로저)
  - 클릭 시 해당 값을 출력
    - 이벤트 객체와 충돌 없음, this도 문제 없음
- $li.addEventListener('click', alertFruit(fruit));
  - alertFruit(fruit) 즉시 실행
  - 반환값은 undefined
  - `addEventListener`는 함수를 전달받아 나중에 실행하지만 `()`를 붙이면 이미 실행된 결과
  - 따라서 고차함수 필요!
...
> **가능!**
> 
> 고차 함수는 '함수를 반환하거나 인자로 받는 함수' → 어떤 문법을 쓰느냐와는 관련 없음
> 즉, 문법이 아닌 **함수의 역할**
> 
> 단, 외부 값을 넘겨줘야 한다면 고차함수나 bind, 클로저 함수로 따로 함수를 만드는 게 좋음
> 실행 시점에 값 고정을 하기 위해!    
...
## 5-4. 정리
- 클로저 : 어떤 함수에서 선언한 변수를 참조하는 내부 함수를 외부로 전달할 경우, 함수의
실행 컨텍스트가 종료된 후에도 해당 변수가 사라지지 않는 현상
  - 내부 함수를 외부로 전달하는 방법 : 함수 return, 콜백 전달
  - 메모리를 계속 차지하므로 더 이상 사용하지 않는 클로저는 소모시키는 관리 필요
 
### 5-4-1. 스코프 체인과 클로저 구분하기!
- 스코프 체인(Scope Chain)
  - 현재 실행 컨텍스트에서 **변수를 어떻게 탐색하는지** 나타내는 구조
  - 시점 : **변수를 참조할 때마다**(상위 스코프로 따라가며 찾음)
  - 주요 대상 : 변수 탐색 경로(Lexical Environment, outer reference)
  - 메모리 영향 : 없음(일시적인 탐색 구조)
  - ✨ 클로저와의 관계
    - 클로저가 동작하기 위한 전제 조건으로 존재    
- 클로저(Closure)
  - 함수가 선언될 때의 **스코프를 기억**하는 현상
  - 시점 : 함수가 반환된 후에도 외부 변수에 접근하려 할 때
  - 주요 대상 : 외부 함수의 지역 변수를 참조하는 내부 함수
  - 메모리 영향 : 있음(GC가 제거 못하게 변수 유지)
  - ✨ 스코프 체인과의 관계
    - 스코프 체인을 활용해 변수에 접근 가능
   
✔️ 스코프 체인은 "변수를 찾아가는 경로"  
  클로저는 "스코프 체인을 통해 외부 변수를 기억하고 참조할 수 있는 함수"
